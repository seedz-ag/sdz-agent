...
export default async (config: ConfigDatabaseInterface | undefined) => {
  ...
  answers.host = await enquirer
    .prompt<QuestionResponse>({
      ...
      message: `What is your ${chalk.green(chalk.bold("MSSQL"))} host?`,
    })
  ...
  answers.port = await enquirer
    .prompt<QuestionResponse>({
      type: "input",
      name: "response",
      initial: config?.port || "1433",
      message: `What is your ${chalk.green(chalk.bold("MSSQL"))} port?`,
    })
    .then((answer) => answer.response);
  ...
  answers.schema = await enquirer
    .prompt<QuestionResponse>({
      ...
      message: `What is your ${chalk.green(chalk.bold("MSSQL"))} database?`,
    })
    .then((answer) => answer.response);
  ...
  answers.username = await enquirer
    .prompt<QuestionResponse>({
      ...
      message: `What is your ${chalk.green(chalk.bold("MSSQL"))} username?`,
    })
    .then((answer) => answer.response);
  ...
  answers.password = await enquirer
    .prompt<QuestionResponse>({
      ...
      message: `What is your ${chalk.green(chalk.bold("MSSQL"))} password?`,
    })
    .then((answer) => answer.response);
  return answers;
};
